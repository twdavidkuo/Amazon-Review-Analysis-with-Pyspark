...
def read_trim_tsv(path, sample=True, sample_size=500):
    ...
    spark = SparkSession.builder.appName('read').config('spark.executor.instances', str(num_nodes)).getOrCreate()
     # Read DataFrame
...
def count_vectorize(df, colname, merge = False):
    if merge:
        ...
        cv = CountVectorizer(inputCol='all_tokens', outputCol='vectorized_features', minDF=1.0)
        cv_model = cv.fit(df)
